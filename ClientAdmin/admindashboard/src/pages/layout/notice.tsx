...
const HeaderNoticeComponent: FC = () => {
  ...
  const handleDetail = (docId: string, index: string) => {
    axios
      .get<Document>('https://localhost:7241/api/Document1/GetDoc1ById/' + docId)
      .then(res => {
        const doc = res.data;
        ...
        if (selectedFB) {
          ...
          axios
            .put('https://localhost:7241/api/Report/UpdateReport', requestBody)
            .then(response => {
                ...
                !item.read ? (
                  ...
                  <List.Item>
                    <List.Item.Meta
                      avatar={<Avatar src={item.avatar} />}
                      title={
                        <a onClick={() => handleDetail(item.docId, item.index)} style={{ color: 'gray' }}>
                          {item.title}
                        </a>
                      }
                      description={
  ...
  return (
    <>
      <Modal
        ...
        width={1000}
        footer={[
          <Button key="view" type={'primary'} onClick={handViewFileModal}>
            View file
          </Button>,
          ...
          <Row gutter={16}>
            <Col span={12}>
              ...
              <Form.Item label="Status">
                <Select value={selectedDocument?.status} onChange={(value: boolean) => setUpdatedStatus(value)}>
            ...
            <Col span={12}>
              ...
              <Form.Item label="User Name">
                <Input value={selectedDocument?.userFullName} disabled />
              </Form.Item>
